...
public class SuperBankValidationPage {
    ...
    public SuperBankValidationPage(WebDriver driver, WebDriverWait wait) {
        this.driver = driver;
        this.wait = wait;
    ...
    public void handleStripeModals() {
        try {
            ...
            // Debug: see what iframes are available
            debugIframes();
            
            // Try to switch to Stripe iframe
            ...
            try {
                // Try account number field
                WebElement accountNumberField = wait.until(ExpectedConditions.elementToBeClickable(
                    By.cssSelector("input[data-elements-stable-field-name='accountNumber']")));
                accountNumberField.sendKeys("000123456789");
                System.out.println("Entered account number");
            } catch (Exception e) {
                System.out.println("Account number field not found with standard selector: " + e.getMessage());
                
                // Try alternative selectors
                try {
                    WebElement accountNumberField = driver.findElement(By.cssSelector("input[autocomplete='account-number']"));
                    accountNumberField.sendKeys("000123456789");
                    System.out.println("Entered account number using autocomplete selector");
                } catch (Exception ex) {
                    System.out.println("Account number field not found with autocomplete selector: " + ex.getMessage());
                }
            }
            
            // Try routing number field
            try {
                WebElement routingNumberField = driver.findElement(
                    By.cssSelector("input[data-elements-stable-field-name='routingNumber']"));
                routingNumberField.sendKeys("110000000");
                System.out.println("Entered routing number");
            } catch (Exception e) {
                System.out.println("Routing number field not found: " + e.getMessage());
                
                try {
                    WebElement routingNumberField = driver.findElement(By.cssSelector("input[autocomplete='routing-number']"));
                    routingNumberField.sendKeys("110000000");
                    System.out.println("Entered routing number using autocomplete selector");
                } catch (Exception ex) {
                    System.out.println("Routing number field not found with autocomplete selector: " + ex.getMessage());
                }
            }
            ...
            try {
                WebElement continueButton = driver.findElement(
                    By.cssSelector("button[data-testid='bank-account-continue-button']"));
                continueButton.click();
                System.out.println("Clicked continue button");
            } catch (Exception e) {
                System.out.println("Continue button not found with data-testid: " + e.getMessage());
                ...
                try {
                    ...
                    for (WebElement button : buttons) {
                        if (button.getText().contains("Continue") || button.getText().contains("Continue")) {
                            button.click();
                            System.out.println("Clicked continue button using text selector");
                            break;
                        }
                    }
                } catch (Exception ex) {
                    System.out.println("Continue button not found with text selector: " + ex.getMessage());
                }
            }
