...
public class SuperBankPaymentValidationPage {
    ...
    public SuperBankPaymentValidationPage(WebDriver driver, WebDriverWait wait) {
        this.driver = driver;
        this.wait = wait;
   ...
   /* 
    public void selectTestInstitution() throws Exception {
        try {
            //By[] backLocators = {
            	    // By.xpath("//div[contains(text(), 'Test Institution')]")z
            	     //By.xpath("//img[contains(@src, 'testmodeGreenBank'
    ...
    */
    public void selectTestInstitution() throws Exception {
        try {
            ...
            // ✅ Define multiple locator strategies
            By[] institutionLocators = {
                By.xpath("//div[contains(text(), 'Test Institution')]"),
                By.xpath("//img[contains(@src, 'testmodeGreenBank')]")
            };

            WebElement testInstitution = null;

            // ✅ Try each locator until one is found & clickable
            for (By locator : institutionLocators) {
                try {
                    testInstitution = wait.until(ExpectedConditions.elementToBeClickable(locator));
                    if (testInstitution.isDisplayed()) {
                        js.executeScript("arguments[0].scrollIntoView(true);", testInstitution);
                        js.executeScript("arguments[0].click();", testInstitution);
                        System.out.println("✅ Selected 'Test Institution' using locator: " + locator);
                        break;
                    }
                } catch (Exception inner) {
                    System.out.println("⚠️ Locator not clickable: " + locator + " | " + inner.getMessage());
                }
            }

            if (testInstitution == null) {
                throw new Exception("❌ 'Test Institution' element not found using any locator!");
            }

            // ✅ Wait for the next modal/screen to load
            Thread.sleep(5000);
