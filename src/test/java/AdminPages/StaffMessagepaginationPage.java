...
public class StaffMessagepaginationPage {
    ...
    private By staffCategory = By.xpath("//div[contains(text(),'Staff')]");
    ...
    public StaffMessagepaginationPage(WebDriver driver) {
        this.driver = driver;
    ...
    public void clickOnMessageTab() {
        ...
        WebElement messages = wait.until(
                ExpectedConditions.presenceOfElementLocated(messagesTab)
        );

        ((JavascriptExecutor) driver).executeScript(
                "arguments[0].scrollIntoView(true);", messages
        );

        wait.until(ExpectedConditions.elementToBeClickable(messages)).click();

        System.out.println("✅ Messages tab clicked");
    }

    
    
    public void clickStaffTab() {
        By staffTab = By.xpath("//div[normalize-space()='Staff']");
        wait.until(ExpectedConditions.elementToBeClickable(staffTab)).click();
        waitForMessageList();
        System.out.println("✅ Staff tab opened");
    }
    ...
    public void searchStaffMessage(String searchText) {
        ...
        System.out.println("🔍 Searched staff message: " + searchText);
    }
