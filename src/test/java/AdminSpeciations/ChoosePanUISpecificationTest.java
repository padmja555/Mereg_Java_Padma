...
public class ChoosePanUISpecificationTest {
    ...
    private IndidualChildChoosePlanPage childchoosePlanPage;
    ...
    public void setUp() {
        ...
        childchoosePlanPage = new IndidualChildChoosePlanPage(driver);
    }

    @Test
    public void completeEnrollmentFlowWithChoosePlanTest() {
        System.out.println("🚀 Starting Complete Enrollment Flow Test (Login → Child → Parent → Physician → Choose Plan)...");
        
        try {
            // STEP 1: Login and Navigate to Enrollment
            ...
            // STEP 4: Comprehensive Physician Information
            validatePhysicianInfoPageUI();
            fillPhysicianInfoFormWithUIValidation();
            validateEmergencyCareSection();
            validatePhysicianFormSubmission();
            
            // STEP 5: Choose Plan and Complete Registration
            validateChoosePlanPage();
            selectPlanAndCompleteRegistration();
            
            System.out.println("✅ Complete Enrollment Flow Test completed successfully!");
            
        } catch (Exception e) {
            System.out.println("❌ Complete Enrollment Flow Test failed: " + e.getMessage());
            takeScreenshot("complete-enrollment-flow-failure");
            Assert.fail("Complete enrollment flow test failed: " + e.getMessage());
        }
    }
    ...
    private void performLoginAndNavigation() {
        System.out.println("\n🔐 STEP 1: Performing Login and Navigation...");
    ...
    // ==================== STEP 2: CHILD ENROLLMENT ====================

    private void completeBasicChildEnrollment() {
        System.out.println("\n👶 STEP 2: Completing Basic Child Enrollment...");
    ...
    private void completeParentGuardianInformation() {
        System.out.println("\n👨‍👩‍👧‍👦 STEP 3: Completing Parent/Guardian Information...");
    ...
    private void validatePhysicianInfoPageUI() {
        System.out.println("\n📋 STEP 4: Validating Physician Information Page UI Structure...");
    ...
    private void validatePhysicianFormSubmission() {
        ...
        try {
            ...
            if (proceedBtn.isEnabled()) {
                ...
                // Wait for navigation to Choose Plan page
                waitForChoosePlanPageToLoad();
            } else {
    ...
    // ==================== STEP 5: CHOOSE PLAN PAGE ====================

    private void waitForChoosePlanPageToLoad() {
        System.out.println("⏳ Waiting for Choose Plan page to load...");
        
        try {
            // Wait for plan elements to be present
            wait.until(driver -> {
                try {
                    List<WebElement> planElements = driver.findElements(childchoosePlanPage.planToggles);
                    System.out.println("📊 Plan elements found: " + planElements.size());
                    return planElements.size() >= 2; // Expecting at least 2 plan options
                } catch (Exception e) {
                    return false;
                }
            });
            ...
            safeSleep(2000);
            System.out.println("✅ Choose Plan page loaded successfully");
            
        } catch (Exception e) {
            System.out.println("❌ Choose Plan page loading failed: " + e.getMessage());
            throw e;
        }
    }
    /*
	...
	*/
    ...
    private void validateChoosePlanPage() {
        System.out.println("\n📋 STEP 5: Validating Choose Plan Page...");
        
        try {
            // Verify page header
            String headerText;
			try {
				headerText = IndidualChildChoosePlanPage.VerifyChoosePlanPage();
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
            Assert.assertTrue(headerText.contains("Choose Your Plan") || 
                            headerText.contains("Select a plan") || 
                            headerText.contains("Plan"),
                "Choose Plan page header not found. Actual: " + headerText);
            System.out.println("✅ Choose Plan page header validated: " + headerText);
            
            // Verify plan options are present
            boolean plansLoaded = childchoosePlanPage.isPlanPageLoaded();
            Assert.assertTrue(plansLoaded, "Choose Plan page not loaded properly");
            System.out.println("✅ Choose Plan page loaded successfully");
            
        } catch (Exception e) {
            throw new RuntimeException("Choose Plan page validation failed: " + e.getMessage());
        }
    }

    private void selectPlanAndCompleteRegistration() {
        System.out.println("\n💰 Selecting Plan and Completing Registration...");
        
        try {
            // STEP 1: Select a random plan
        	childchoosePlanPage.selectRandomMonthPlan();
            safeSleep(1000);
            
            // STEP 2: Verify plan selection
            String selectedPlan = childchoosePlanPage.getSelectedPlanText();
            System.out.println("✅ Currently Selected Plan: " + selectedPlan);
            
            // STEP 3: Click Complete Registration button
            childchoosePlanPage.ProceedToCompleteRegestration();
            
            // STEP 4: Verify registration completion
            verifyRegistrationCompletion();
            
            System.out.println("✅ Plan selection and registration completed successfully!");
            
        } catch (Exception e) {
            throw new RuntimeException("Plan selection and registration failed: " + e.getMessage());
        }
    }

    private void verifyRegistrationCompletion() {
        System.out.println("⏳ Verifying Registration Completion...");
        ...
        try {
            ...
            boolean isSuccess = checkForSuccessIndicators();
            
            if (isSuccess) {
                System.out.println("✅ Registration completed successfully!");
            } else {
                System.out.println("⚠️ Registration may have completed, but no clear success indicator found");
            }
            ...
            System.out.println("⚠️ Registration completion verification had issues: " + e.getMessage());
        }
    }
    ...
    private boolean checkForSuccessIndicators() {
        try {
            // Check for various success indicators
            List<WebElement> successIndicators = driver.findElements(
                By.xpath("//*[contains(text(), 'success') or contains(text(), 'Success') or " +
                         "contains(text(), 'complete') or contains(text(), 'Complete') or " +
                         "contains(text(), 'thank you') or contains(text(), 'Thank You') or " +
                         "contains(text(), 'confirmation') or contains(text(), 'Confirmation')]")
            );
            
            if (!successIndicators.isEmpty()) {
                System.out.println("✅ Success indicator found: " + successIndicators.get(0).getText());
                return true;
            }
            ...
            if (currentUrl.contains("success") || currentUrl.contains("complete") || 
                currentUrl.contains("confirmation") || currentUrl.contains("thank")) {
                System.out.println("✅ Success indicator in URL: " + currentUrl);
                return true;
            }
            ...
            System.out.println("⚠️ Error checking success indicators: " + e.getMessage());
            return false;
        }
    }
    ...
    public void tearDown() {
        System.out.println("\nComplete Enrollment Flow Test completed! Closing browser...");
        safeSleep(3000);
        if (driver != null) {
            driver.quit();
